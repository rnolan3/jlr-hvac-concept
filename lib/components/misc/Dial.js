...
export default class Dial extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    if (nextProps.defaultValue !== this.state.value) {
      this.setState({
        deg: (nextProps.defaultValue + nextProps.min) / (nextProps.max - nextProps.min) * 100,
  ...
  animateArc = (ctx, endPerc = 360, current = 0, styles) => {
    this.drawArc(ctx, current, styles)

    if (current < endPerc) {
      requestAnimationFrame(() => this.animateArc(ctx, endPerc, ++current, styles))
    }
  };

  animateBg = (current = 0) => {
    let rangeLine = current < this.props.maxArc ? current : this.props.maxArc
    let styles = {
  ...
  handleTouchMove = (event) => {
    ...
    let degrees =  (v + this.props.min) / (this.props.max - this.props.min) * 100
